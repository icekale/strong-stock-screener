...
import { Alert, Button, Empty, Progress, Select, Skeleton, Space, Table, Tag, Typography, message } from "antd";
...
export default function SentimentPage() {
  ...
  async function refresh(date = tradeDate, forceRefresh = false) {
    ...
    setError(null);
    const summaryRequest = getSentimentSummary(date, 80, forceRefresh)
      .then(setSummary)
      .catch((err: unknown) => {
    ...
    const detailRequest = getSentimentDetail(date, 80, forceRefresh)
      .then((nextDetail) => {
    ...
    await Promise.allSettled([summaryRequest, detailRequest]);
  }
...
function IntradaySentimentPanel({
  data,
  digest,
  digestLoading,
  loading,
