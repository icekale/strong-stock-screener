    ...
    <article className="screener-metric screener-metric--breadth">
      ...
      <div className="screener-metric__value mt-2 flex items-baseline gap-1 text-3xl font-black leading-none tabular-nums">
        {advanceCount === null || declineCount === null ? (
          ...
          <>
            <span className="text-[var(--market-rise)]">{advanceCount}</span>
            <span className="text-[var(--app-muted)]">/</span>
            <span className="market-green-text">{declineCount}</span>
          </>
        )}
      </div>
      <p className="mt-3 text-xs font-medium text-[var(--app-muted)]">{subValue}</p>
      <div className="mt-3 flex h-1.5 overflow-hidden rounded-full bg-[var(--app-border)]">
        <div className="h-full bg-[var(--market-rise)]" style={{ width: `${upWidth}%` }} />
        <div className="h-full market-green-fill" style={{ width: `${downWidth}%` }} />
      </div>
      <div className="mt-4 flex items-center justify-between border-t border-[var(--app-border)] pt-3 text-xs">
        <span className="text-[var(--app-muted)]">{footerLabel}</span>
        <span className="font-semibold text-[var(--app-muted)]">{footerValue}</span>
      </div>
    </article>
  );
}
...
function MarketMetric({
  ...
  return (
    <article className="screener-metric">
      ...
      <p className="mt-3 text-xs font-medium text-[var(--app-muted)]">{subValue}</p>
      {progress !== undefined && (
        <div className="mt-3 h-1.5 rounded-full bg-[var(--app-border)]">
          <div className="h-1.5 rounded-full market-green-fill" style={{ width: `${Math.max(0, Math.min(100, progress))}%` }} />
        </div>
      )}
      <div className="mt-4 flex items-center justify-between border-t border-[var(--app-border)] pt-3 text-xs">
        <span className="text-[var(--app-muted)]">{helper ?? footerLabel}</span>
    ...
    <section className="rounded-md border border-[var(--app-border)] bg-[var(--app-raised)] p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs font-medium text-[var(--app-muted)]">等待非资金流口径模型</p>
        </div>
        <Tag className="m-0">规划中</Tag>
      </div>
      <div className="mt-4 flex min-h-[214px] items-center justify-center rounded-lg border border-dashed border-[var(--app-border)] bg-white/55 px-5 text-center">
        <div>
          <div className="text-sm font-black text-[var(--app-ink)]">板块强度待接入</div>
          <p className="mt-2 max-w-[280px] text-xs leading-5 text-[var(--app-muted)]">
...
function SectorFlowStat({
  ...
  const toneClass = tone === "red" ? "text-[var(--market-rise)]" : tone === "green" ? "market-green-text" : "text-[var(--app-ink)]";
  return (
  ...
  return (
    <div className="min-w-0">
      <div className="mb-2 flex items-center justify-between text-xs">
        <span className="font-black text-[var(--app-ink)]">{title}</span>
        <span className="font-semibold text-[var(--app-muted)]">{items.length || "待数据"}</span>
      </div>
      <div className="space-y-2">
        {items.length > 0 ? (
          items.map((item) => <SectorFlowRow item={item} key={`${title}-${item.name}`} maxFlow={maxFlow} tone={tone} />)
        ) : (
          <div className="rounded-lg border border-dashed border-[var(--app-border)] px-3 py-6 text-center text-xs font-bold text-[var(--app-muted)]">
            板块资金流暂不可用
          </div>
        )}
      </div>
...
function SectorFlowRow({
  ...
  const barClass = tone === "red" ? "bg-[var(--market-rise)]" : "market-green-fill";
  const valueClass = tone === "red" ? "text-[var(--market-rise)]" : "market-green-text";

  return (
    <div className="grid grid-cols-[96px_minmax(0,1fr)_80px] items-center gap-3 text-sm">
      <span className="truncate font-bold text-[var(--app-ink)]" title={`${item.name} · ${item.leader ?? "暂无领涨股"}`}>
        {item.name}
      </span>
      <div className="min-w-0">
        ...
        <div className="mt-1 truncate text-[10px] font-semibold text-[var(--app-muted)]">
