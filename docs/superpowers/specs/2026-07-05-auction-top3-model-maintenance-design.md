...
5. 提供可复制的 Codex 分析提示词和维护包摘要，方便用户把结果交给当前 Codex 线程继续分析。
...
- 不让 Web 应用直接控制 Codex Desktop 线程；第一版只提供可复制的结构化提示词。
...
同时新增 `复制 Codex 分析提示词`，把维护包摘要和用户意图整理成一段可直接粘贴给 Codex 的中文提示词。
...
- 维护包不保存 API Key、完整 K 线、完整竞价全市场数据。
...
3. `Codex 提示词`
   - 提供复制按钮。
   - 文案示例：`请基于以下模型维护包，分析 GSGF 和竞价 Top3 模型是否退化，并给出只观察不自动改规则的建议。`
...
- 生成维护包成功：提示维护包已生成，并显示“下一步：提交给 AI 分析”。
